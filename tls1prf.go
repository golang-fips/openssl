...
func tls1PRF1(result, secret, label, seed []byte, md ossl.EVP_MD_PTR) error {
	...
	if err := ossl.EVP_PKEY_CTX_ctrl(ctx, -1,
		ossl.GO1_EVP_PKEY_OP_DERIVE,
	...
	outLen := len(result)
	if outLen, err = ossl.EVP_PKEY_derive_wrapper(ctx, base(result), outLen); err != nil {
		return err
	}
...
var fetchTLS1PRF3 = sync.OnceValues(func() (ossl.EVP_KDF_PTR, error) {
	...
	kdf, err := ossl.EVP_KDF_fetch(nil, cStringData(ossl.OSSL_KDF_NAME_TLS1_PRF), nil)
	if err != nil {
		return nil, err
	}
	return kdf, nil
...
func tls1PRF3(result, secret, label, seed []byte, md ossl.EVP_MD_PTR) error {
	...
	bld.addOctetString(ossl.OSSL_KDF_PARAM_SECRET, secret)
	bld.addOctetString(ossl.OSSL_KDF_PARAM_SEED, label)
	bld.addOctetString(ossl.OSSL_KDF_PARAM_SEED, seed)
	...
	return ossl.EVP_KDF_derive(ctx, base(result), len(result), params)
}
