...
func newECDSAKey3(nid C.int, bx, by, bd C.GO_BIGNUM_PTR) (C.GO_EVP_PKEY_PTR, error) {
	...
	defer bld.free()
	bld.addUtf8String(OSSL_PKEY_PARAM_GROUP_NAME, C.go_openssl_OBJ_nid2sn(nid), 0)
	bld.addOctetString(OSSL_PKEY_PARAM_PUB_KEY, pubBytes)
	var selection C.int
	if bd != nil {
		bld.addBN(OSSL_PKEY_PARAM_PRIV_KEY, bd)
		selection = C.GO_EVP_PKEY_KEYPAIR
	} else {
		selection = C.GO_EVP_PKEY_PUBLIC_KEY
	}
	return newEvpFromParams(C.GO_EVP_PKEY_EC, selection, bld)
}
