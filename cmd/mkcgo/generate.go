package main

import (
	"fmt"
	"io"
	"slices"
	"strconv"
	"strings"
)

// generateGo output Go source file from src.
func (src *source) generateGo(w io.Writer) {
	// Output header notice and package declaration.
	fmt.Fprintf(w, "// Code generated by mkcgo. DO NOT EDIT.\n\n")
	fmt.Fprintf(w, "package %s\n\n", *packageName)

	// This block outputs C header includes and forward declarations for loader functions.
	fmt.Fprintf(w, "/*\n")
	if *includeHeader != "" {
		fmt.Fprintf(w, "#include \"%s\"\n", *includeHeader)
	}
	for _, file := range src.files {
		fmt.Fprintf(w, "#include %q\n", file)
	}
	for _, tag := range src.tags() {
		fmt.Fprintf(w, "void __mkcgoLoad_%s(void* handle);\n", tag)
	}
	fmt.Fprintf(w, "*/\n")
	fmt.Fprintf(w, "import \"C\"\n")
	if len(src.stdLibImports) > 0 {
		fmt.Fprintf(w, "import (\n")
		for _, imp := range src.stdLibImports {
			fmt.Fprintf(w, "\t%q\n", imp)
		}
		fmt.Fprintf(w, ")\n\n")
	}

	// Generate type aliases for all
	src.generateGoAliases(w)

	// Generate Go wrapper functions that load the C symbols.
	for _, tag := range src.tags() {
		fmt.Fprintf(w, "func mkcgoLoad_%s(handle unsafe.Pointer) {\n", tag)
		fmt.Fprintf(w, "\tC.__mkcgoLoad_%s(handle)\n", tag)
		fmt.Fprintf(w, "}\n\n")
	}

	// Generate function wrappers.
	for _, fn := range src.funcs {
		src.generateGoFn(fn, w)
	}

}

// generateGoAliases generates Go type aliases for C types.
func (src *source) generateGoAliases(w io.Writer) {
	// Traverse all functions and collect the unique C types.
	for _, f := range src.funcs {
		handleType := func(typ string) {
			if typ == "" {
				return
			}
			typ = strings.TrimPrefix(typ, "const ")
			typ = strings.TrimRight(typ, "*")
			if _, ok := src.ctypes[typ]; ok {
				return
			}
			src.ctypes[typ] = struct{}{}
		}
		for _, p := range f.params {
			handleType(p.typ)
		}
		handleType(f.rets.typ)
	}
	types := make([]string, 0, len(src.ctypes))
	for typ := range src.ctypes {
		types = append(types, typ)
	}
	slices.Sort(types)
	for _, typ := range types {
		if isStdType(typ) {
			// Skip standard types.
			continue
		}
		fmt.Fprintf(w, "type %s = C.%s\n", typ, typ)
	}
}

// generateC creates the C source file content.
func (src *source) generateC(w io.Writer) {
	// Header and includes.
	fmt.Fprintf(w, "// Code generated by mkcgo. DO NOT EDIT.\n\n")
	if *includeHeader != "" {
		fmt.Fprintf(w, "#include \"%s\"\n", *includeHeader)
	}
	for _, file := range src.files {
		fmt.Fprintf(w, "#include %q\n", file)
	}
	fmt.Fprintf(w, "#include <stddef.h>\n")
	fmt.Fprintf(w, "#include <stdlib.h>\n")
	fmt.Fprintf(w, "#include <stdint.h>\n")
	fmt.Fprintf(w, "#include <stdio.h>\n")
	fmt.Fprintf(w, "\n")

	// Platform-specific includes.
	fmt.Fprintf(w, "#ifdef _WIN32\n")
	fmt.Fprintf(w, "#include <windows.h>\n")
	fmt.Fprintf(w, "#define dlsym GetProcAddress\n")
	fmt.Fprintf(w, "#else\n")
	fmt.Fprintf(w, "#include <dlfcn.h>\n")
	fmt.Fprintf(w, "#endif\n\n")

	// Function pointer declarations.
	for _, fn := range src.funcs {
		if fn.variadic {
			continue
		}
		fmt.Fprintf(w, "%s (*_g_%s)(%s);\n", fn.rets.typ, fn.cName, fn.toCArgs(false, true))
	}
	fmt.Fprintf(w, "\n")

	fmt.Fprintf(w, "#define __mkcgo__dlsym(name) __mkcgo__dlsym2(name, name)\n\n")

	fmt.Fprintf(w, "#define __mkcgo__dlsym2(cname, importname)								\\\n")
	fmt.Fprintf(w, "\t_g_##cname = (typeof(_g_##cname))dlsym(handle, #cname);				\\\n")
	fmt.Fprintf(w, "\tif (_g_##cname == NULL) {												\\\n")
	fmt.Fprintf(w, "\t\tfprintf(stderr, \"Cannot get required symbol \" #cname \"\\n\");	\\\n")
	fmt.Fprintf(w, "\t\tabort();															\\\n")
	fmt.Fprintf(w, "\t}\n\n")

	// Loader functions for each tag.
	for _, tag := range src.tags() {
		fmt.Fprintf(w, "void __mkcgoLoad_%s(void* handle) {\n", tag)
		for _, fn := range src.funcs {
			if fn.variadic || fn.tag != tag {
				continue
			}
			if fn.cName == fn.importName {
				fmt.Fprintf(w, "\t__mkcgo__dlsym(%s)\n", fn.cName)
			} else {
				fmt.Fprintf(w, "\t__mkcgo__dlsym2(%s, %s)\n", fn.cName, fn.importName)
			}
		}
		fmt.Fprintf(w, "}\n\n")
	}

	// Generate C function wrappers.
	for _, f := range src.funcs {
		src.generateCFn(f, w)
	}
}

// generateGoFn generates Go function f.
func (src *source) generateGoFn(f *fn, w io.Writer) {
	fmt.Fprintf(w, "func %s(%s)", f.goName, f.toGoParams())
	ret := f.rets.toGoParams()
	if len(ret) > 0 {
		fmt.Fprintf(w, " %s ", ret)
	}
	fmt.Fprintf(w, "{\n")
	fmt.Fprintf(w, "\t")
	if !f.rets.isVoid() {
		fmt.Fprintf(w, "r0 := ")
	}
	fmt.Fprintf(w, "C.%s(%s)\n", f.cName, f.toGoArgs())
	if !f.rets.isVoid() {
		errSrc := f.rets.toGoSource()
		if len(errSrc) > 0 {
			fmt.Fprintf(w, "\t%s\n", errSrc)
		}
		fmt.Fprintf(w, "\treturn\n")
	}
	fmt.Fprintf(w, "}\n\n")
}

func (src *source) generateCFn(f *fn, w io.Writer) {
	if f.variadic {
		return
	}
	fmt.Fprintf(w, "%s %s(%s) {\n\t", f.rets.typ, f.cName, f.toCArgs(true, true))
	if f.rets.typ != "void" {
		fmt.Fprintf(w, "return ")
	}
	fmt.Fprintf(w, "_g_%s(%s);\n", f.cName, f.toCArgs(true, false))
	fmt.Fprintf(w, "}\n\n")
}

// toGo converts C parameter p to Go parameter.
func (p *param) toGo() string {
	goType := cArgToCgo(p.typ)
	var s string
	switch {
	case goType == "":
		return ""
	case goType == "unsafe.Pointer":
		return p.name
	case goType[0] == '*':
		return fmt.Sprintf("(%s)(unsafe.Pointer(%s))", goType, p.name)
	default:
		s = p.name
	}
	return fmt.Sprintf("%s(%s)", goType, s)
}

func isStdType(t string) bool {
	switch t {
	case "int8_t", "uint8_t", "int16_t", "uint16_t", "int32_t", "uint32_t", "int64_t", "uint64_t",
		"int", "unsigned int", "long", "unsigned long", "size_t", "uintptr_t", "char", "unsigned char",
		"void", "unsigned long long", "long long", "signed char":
		return true
	}
	return false
}

// cArgToCgo converts C parameter type t to a cgo type,
// that is, a Go type that can be used in a cgo call.
func cArgToCgo(t string) string {
	t, _ = strings.CutPrefix(t, "const ")
	if t == "void" {
		return ""
	}
	if strings.Contains(t, "void*") {
		return "unsafe.Pointer"
	}
	if strings.HasSuffix(t, "*") {
		// Remove all trailing '*' characters.
		var i, n int
		for i = len(t) - 1; i >= 0; i-- {
			if t[i] != '*' {
				break
			}
			n++
		}
		s := cArgToCgo(t[:i+1])
		s = strings.Repeat("*", n) + s
		return s
	}
	// Map C types with spaces to cgo special-cased types.
	switch t {
	case "signed char":
		t = "schar"
	case "unsigned char":
		t = "uchar"
	case "unsigned short":
		t = "ushort"
	case "unsigned int":
		t = "uint"
	case "unsigned long":
		t = "ulong"
	case "long long":
		t = "longlong"
	case "unsigned long long":
		t = "ulonglong"
	}
	return "C." + t
}

// cTypeToGo converts C type t to a Go type.
func cTypeToGo(t string) string {
	t, _ = strings.CutPrefix(t, "const ")
	if t == "void" {
		return ""
	}
	if strings.HasSuffix(t, "void*") {
		return "unsafe.Pointer"
	}
	if strings.HasSuffix(t, "*") {
		// Remove all trailing '*' characters.
		var i, n int
		for i = len(t) - 1; i >= 0; i-- {
			if t[i] != '*' {
				break
			}
			n++
		}
		s := cTypeToGo(t[:i+1])
		s = strings.Repeat("*", n) + s
		return s
	}
	var s string
	switch t {
	case "int8_t":
		s = "int8"
	case "uint8_t":
		s = "uint8"
	case "int16_t":
		s = "int16"
	case "uint16_t":
		s = "uint16"
	case "int32_t":
		s = "int32"
	case "uint32_t":
		s = "uint32"
	case "int64_t":
		s = "int64"
	case "uint64_t":
		s = "uint64"
	case "int":
		s = "int32"
	case "unsigned int":
		s = "uint32"
	case "long":
		s = "int64"
	case "unsigned long":
		s = "uint64"
	case "size_t":
		s = "int"
	case "uintptr_t":
		s = "uintptr"
	case "char", "unsigned char":
		s = "byte"
	default:
		s = t
	}
	return s
}

// toC returns C source code of parameter p.
func (p *param) toC(paramNames, paramTypes bool) string {
	var s string
	if paramTypes {
		s += p.typ
	}
	if paramNames && p.typ != "void" {
		if len(s) > 0 {
			s += " "
		}
		s += "_arg" + strconv.Itoa(p.tmpVarIdx)
	}
	return s
}

// toGoParams returns source code of return parameters.
func (r *rets) toGoParams() string {
	if r.isVoid() {
		return ""
	}
	return fmt.Sprintf("(%s %s)", r.name, cTypeToGo(r.typ))
}

func (r *rets) isVoid() bool {
	return r.typ == "void"
}

// toGoSource returns Go source code that sets return parameters.
func (r *rets) toGoSource() string {
	if r.isVoid() {
		return ""
	}
	goType := cTypeToGo(r.typ)
	s := ""
	switch {
	case goType[0] == '*':
		s = fmt.Sprintf("%s = (%s)(unsafe.Pointer(r0))", r.name, goType)
	default:
		s = fmt.Sprintf("%s = %s(r0)", r.name, goType)
	}
	return s
}

// toGoParams returns source code for function f parameters.
func (f *fn) toGoParams() string {
	return join(f.params, func(p *param) string { return p.name + " " + cTypeToGo(p.typ) }, ", ")
}

// argList returns source code for C parameters for function f.
func (f *fn) toGoArgs() string {
	a := make([]string, 0, len(f.params))
	for _, p := range f.params {
		a = append(a, p.toGo())
	}
	a = slices.DeleteFunc(a, func(s string) bool {
		return s == ""
	})
	return strings.Join(a, ", ")
}

// toCArgs returns source code for C parameters for function f.
func (f *fn) toCArgs(paramNames, paramTypes bool) string {
	a := make([]string, 0, len(f.params))
	for _, p := range f.params {
		a = append(a, p.toC(paramNames, paramTypes))
	}
	a = slices.DeleteFunc(a, func(s string) bool {
		return s == ""
	})
	return strings.Join(a, ", ")
}

// join concatenates parameters ps into a string with sep separator.
// Each parameter is converted into string by applying fn to it
// before conversion.
func join(ps []*param, fn func(*param) string, sep string) string {
	if len(ps) == 0 {
		return ""
	}
	a := make([]string, 0, len(ps))
	for _, p := range ps {
		a = append(a, fn(p))
	}
	return strings.Join(a, sep)
}
