...
func escapePtrName() string {
	...
	return "_mkcgoEscapePtr_" + baseName
}
...
func generateGoNocgo(src *mkcgo.Source, w io.Writer) {
	...
	// The name is derived from the output file to avoid redeclarations.
	escName := escapePtrName()
	fmt.Fprintf(w, "var _mkcgoAlwaysFalse_%s bool\n", escName)
	fmt.Fprintf(w, "var _mkcgoEscapeSink_%s unsafe.Pointer\n\n", escName)
	fmt.Fprintf(w, "// %s forces p to escape to the heap.\n", escName)
	...
	fmt.Fprintf(w, "func %s(p unsafe.Pointer) unsafe.Pointer {\n", escName)
	fmt.Fprintf(w, "\tif _mkcgoAlwaysFalse_%s {\n", escName)
	fmt.Fprintf(w, "\t\t_mkcgoEscapeSink_%s = p\n", escName)
	fmt.Fprintf(w, "\t}\n")
...
func generateNocgoFnBody(src *mkcgo.Source, fn *mkcgo.Func, errorType int, newR0 bool, w io.Writer) {
	...
	for _, param := range fn.Params {
		...
		if _, ok := fn.SliceFromPtr(param.Name); ok {
			...
			if needEscape {
				fmt.Fprintf(w, ", uintptr(%s(unsafe.Pointer(unsafe.SliceData(%s))))", escapePtrName(), param.Name)
			} else {
			...
			if needEscape {
				fmt.Fprintf(w, ", uintptr(%s(unsafe.Pointer(%s)))", escapePtrName(), param.Name)
			} else {
