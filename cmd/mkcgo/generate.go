...
// escapePtrSuffix returns a unique suffix for the mkcgoEscapePtr helper function
// based on the output file name to avoid redeclarations in packages with
// multiple generated nocgo files.
// Examples: zdl_nocgo.go -> Dl, zossl_nocgo.go -> Ossl
func escapePtrSuffix() string {
	if *fileName == "" {
		return ""
	}
	baseName := strings.TrimSuffix(*fileName, ".go")
	baseName = strings.TrimPrefix(baseName, "z")
	baseName = strings.TrimSuffix(baseName, "_nocgo")
...
func fnParamByName(fn *mkcgo.Func, name string) (*mkcgo.Param, bool) {
	for _, p := range fn.Params {
		if p.Name == name {
			return p, true
		}
...
func isPointerParam(param *mkcgo.Param) bool {
	return param != nil && strings.Contains(param.Type, "*")
}

func shouldSkipSliceLenParam(fn *mkcgo.Func, param *mkcgo.Param) bool {
	if _, ok := fn.SliceFromLen(param.Name); !ok {
		return false
	}
	return !isPointerParam(param)
}

func generateSliceLenPointerChecks(fn *mkcgo.Func, w io.Writer) {
	for _, slice := range fn.Slice {
		lenParam, ok := fnParamByName(fn, slice.Len)
...
func fnToGoParams(fn *mkcgo.Func) string {
	return join(fn.Params, func(_ int, p *mkcgo.Param) string {
		if shouldSkipSliceLenParam(fn, p) {
...
func fnToGoArgs(fn *mkcgo.Func) string {
	return join(fn.Params, func(_ int, p *mkcgo.Param) string {
		...
		if slice, ok := fn.SliceFromLen(p.Name); ok {
			if pLen, found := fnParamByName(fn, slice.Len); found && isPointerParam(pLen) {
				arg = p.Name
			} else {
				arg = "len(" + slice.Ptr + ")"
			}
...
func generateGoNocgo(src *mkcgo.Source, w io.Writer) {
	...
	// The suffix is derived from the output file to avoid redeclarations.
	suffix := escapePtrSuffix()
	fmt.Fprintf(w, "var _mkcgoAlwaysFalse%s bool\n", suffix)
	fmt.Fprintf(w, "var _mkcgoEscapeSink%s unsafe.Pointer\n\n", suffix)
	fmt.Fprintf(w, "// mkcgoEscapePtr%s forces p to escape to the heap.\n", suffix)
	...
	fmt.Fprintf(w, "func mkcgoEscapePtr%s(p unsafe.Pointer) unsafe.Pointer {\n", suffix)
	fmt.Fprintf(w, "\tif _mkcgoAlwaysFalse%s {\n", suffix)
	fmt.Fprintf(w, "\t\t_mkcgoEscapeSink%s = p\n", suffix)
	fmt.Fprintf(w, "\t}\n")
...
func generateNocgoFn(typePtrs map[string]bool, src *mkcgo.Source, fn *mkcgo.Func, w io.Writer) {
	...
	for _, param := range fn.Params {
		...
		if shouldSkipSliceLenParam(fn, param) {
			// Skip length parameter for slice.
			continue
		}
		if paramCount > 0 {
...
func generateNocgoFnBody(src *mkcgo.Source, fn *mkcgo.Func, errorType int, newR0 bool, w io.Writer) {
	...
	for _, param := range fn.Params {
		...
		if _, ok := fn.SliceFromPtr(param.Name); ok {
			...
			if needEscape {
				fmt.Fprintf(w, ", uintptr(mkcgoEscapePtr%s(unsafe.Pointer(unsafe.SliceData(%s))))", escapePtrSuffix(), param.Name)
			} else {
			...
			if pLen, found := fnParamByName(fn, slice.Len); found && isPointerParam(pLen) {
				if needEscape {
					fmt.Fprintf(w, ", uintptr(mkcgoEscapePtr%s(unsafe.Pointer(%s)))", escapePtrSuffix(), param.Name)
				} else {
