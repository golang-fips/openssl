...
int go_openssl_fips_enabled(void *handle) {
  ...
  EVP_default_properties_is_fips_enabled =
      (int (*)(void *))dlsym(handle, "EVP_default_properties_is_fips_enabled");
  OSSL_PROVIDER_available =
      (int (*)(void *, const char *))dlsym(handle, "OSSL_PROVIDER_available");
  if (EVP_default_properties_is_fips_enabled != NULL &&
      OSSL_PROVIDER_available != NULL &&
      EVP_default_properties_is_fips_enabled(NULL) == 1 &&
      OSSL_PROVIDER_available(NULL, "fips") == 1)
    return 1;
...
#define DEFINEFUNC_INTERNAL(name, func)                                        \
  ...
  if (_g_##name == NULL) {                                                     \
    fprintf(stderr,                                                            \
            "Cannot get required symbol " #func                                \
            " from libcrypto version %d.%d\n",                                 \
            major, minor);                                                     \
    abort();                                                                   \
  }
#define DEFINEFUNC(ret, func, args, argscall) DEFINEFUNC_INTERNAL(func, #func)
#define DEFINEFUNC_LEGACY_1_0(ret, func, args, argscall)                       \
  if (major == 1 && minor == 0) {                                              \
...
static unsigned long version_num(void *handle) {
